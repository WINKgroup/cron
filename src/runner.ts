...
export default abstract class CronRunner {
    ...
    constructor(everySeconds:number, inputOptions?:Partial<CronRunnerInput>) {
        ...
        this.consoleLog = options.consoleLog ? options.consoleLog : new ConsoleLog({ prefix: 'CronRunner' })
    ...
    async setup() { // for an eventually intial async setup
        this._setup = true
    } 

    async start() {
        if (!this._setup) await this.setup()
        this._interval = setInterval( async () => this.run(), this.cron.everySeconds * 1000 )
        this._active = true
